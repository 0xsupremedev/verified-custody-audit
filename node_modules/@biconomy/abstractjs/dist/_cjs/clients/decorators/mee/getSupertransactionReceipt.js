...
async function getSupertransactionReceipt(client, parameters) {
    ...
    const metaStatus = await (0, parseTransactionStatus_1.parseTransactionStatus)(explorerResponse.userOps);
    switch (metaStatus.status) {
        ...
        case "MINED_SUCCESS": {
            if (waitForReceipts) {
                receipts = await Promise.all(explorerResponse.userOps
                    .filter((userOp) => {
                    ...
                    .map(({ chainId, executionData }) => (0, actions_1.getTransactionReceipt)(account.deploymentOn(Number(chainId), true).publicClient, { confirmations, ...parameters, hash: executionData })));
            }
            break;
        }
        default: {
    ...
    const explorerLinks = explorerResponse.userOps.reduce((acc, userOp) => {
